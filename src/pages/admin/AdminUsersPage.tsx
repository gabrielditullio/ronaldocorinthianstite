...
// Reuse the existing AdminPage content but wrap in the new AdminLayout
// We'll extract the inner content from AdminPage and use AdminLayout
export default function AdminUsersPage() {
  // For now, render the existing admin user management inside the new layout
  return <AdminPageWithNewLayout />;
}

// We recreate AdminPage logic inside AdminLayout
...
function AdminPageWithNewLayout() {
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="relative max-w-sm">
          ...
          <Input className="pl-9" placeholder="Buscar por email ou nome..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} />
        </div>
                ...
                {isLoading ? (
                  ...
                  paged.map((u) => (
                    <TableRow key={u.id}>
                      ...
                      <TableCell className="text-xs">{u.subscription_activated_at ? new Date(u.subscription_activated_at).toLocaleDateString("pt-BR") : "—"}</TableCell>
                      ...
                      <TableCell>
                        {u.subscription_status === "active" ? (
                          <Button size="sm" variant="outline" onClick={() => setConfirmAction({ userId: u.id, action: "deactivate" })}>Desativar</Button>
                        ) : (
                          <Button size="sm" onClick={() => setConfirmAction({ userId: u.id, action: "activate" })}>Ativar</Button>
                        )}
                      </TableCell>
                    </TableRow>
                  ))
      ...
      <AlertDialog open={!!confirmAction} onOpenChange={() => setConfirmAction(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => { if (confirmAction) mutation.mutate({ userId: confirmAction.userId, action: confirmAction.action }); setConfirmAction(null); }}>Confirmar</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </AdminLayout>
